...
import 'certificate_screen.dart'; // Add this import
...
class _QuizScreenState extends State<QuizScreen> {
  ...
  List<int?> _selectedAnswers = [null, null, null]; // Track selected answers for each question
  ...
  void _finishQuiz() {
    ...
    // Check if user passed (e.g., 70% or higher)
    double percentage = (score / _questions.length) * 100;
    bool passed = percentage >= 70;

    if (passed) {
      // Navigate to CertificateScreen if passed
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (context) => CertificateScreen(
            userName: 'John Dalton', // Hardcoded for now, replace with dynamic user data later
            score: score,
      ...
      // Show score dialog if not passed
      showDialog(
        context: context,
        builder: (context) => AlertDialog(
          title: const Text('Quiz Completed!'),
          content: Text('Your score: $score/${_questions.length}\nSorry, you didn\'t pass (70% required).'),
          actions: [
            TextButton(
              onPressed: () {
